chamadaAjax('php/selectsJson.php?parametro=setores', lista_de_setores);
chamadaAjax('php/selectsJson.php?parametro=setores&setor', alteraTitulodoSetor);
...
function lista_de_setores(data) {
    ...
    for (i = 0; i < data.length; i++) {
        ...
        //add os elem
        link.textContent = data[i].nome_setor;
...
function alteraTitulodoSetor(data) {
    ...
    for (i = 0; i < data.length; i++) {
        ...
        if (id_do_setor === id_do_setor_banco) {
            var nome_do_setor = data[i].nome_setor;
            if (titulo_aba) {
